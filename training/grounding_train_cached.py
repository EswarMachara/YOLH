...
print("\n" + "=" * 70)
print("GROUNDING TRAINING WITH CACHED YOLO FEATURES (SAFE RUN)")
print("=" * 70)
...
# =============================================================================
# TASK 1: TRAINING CONFIGURATION (LOCKED - LAPTOP SAFE)
# =============================================================================

GROUNDING_TRAIN_CONFIG = {
    "num_epochs": 2,
    "batch_size": 1,
    "max_steps_per_epoch": 100,
    "learning_rate": 1e-4,
    "device": "cpu",  # Laptop safe - no GPU
}

print(f"\nConfiguration (LOCKED - LAPTOP SAFE):")
for k, v in GROUNDING_TRAIN_CONFIG.items():
    print(f"  {k}: {v}")
...
device = GROUNDING_TRAIN_CONFIG["device"]
...
coco_json = project_root / "_annotations_COCO_final.json"

dataset = CachedFeatureDataset(
    cache_dir=cache_dir,
    coco_json_path=coco_json,
    max_samples=GROUNDING_TRAIN_CONFIG["max_steps_per_epoch"] * GROUNDING_TRAIN_CONFIG["num_epochs"],
)

dataloader = DataLoader(
    dataset,
    batch_size=GROUNDING_TRAIN_CONFIG["batch_size"],
    shuffle=True,
    num_workers=0,
)
...
optimizer = torch.optim.Adam(trainable_params, lr=GROUNDING_TRAIN_CONFIG["learning_rate"])
print(f"✓ Optimizer initialized (Adam, lr={GROUNDING_TRAIN_CONFIG['learning_rate']})")
...
for epoch in range(GROUNDING_TRAIN_CONFIG["num_epochs"]):
    if early_stopped:
        break
        
    print(f"\n{'='*70}")
    print(f"EPOCH {epoch + 1}/{GROUNDING_TRAIN_CONFIG['num_epochs']}")
    print(f"{'='*70}")
    
    epoch_losses = []
    
    for batch_idx, batch in enumerate(tqdm(dataloader, desc=f"Epoch {epoch+1}")):
        if step_counter >= GROUNDING_TRAIN_CONFIG["max_steps_per_epoch"] * GROUNDING_TRAIN_CONFIG["num_epochs"]:
            break
...
if all_passed or early_stopped:
    print("\n✅ GROUNDING TRAINING PASSED (SAFE RUN)")
    print("\nAll checks passed:")
    if loss_decreased:
